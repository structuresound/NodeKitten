...
void MeshBatcher::chooseShader() {
    ...
    if (_numTextures) {
        if (_is2d) {
            shader = Program::shaderNamed("spriteBatch", {ShaderModule::textureModule(1), ShaderModule::batchSpritePositionModule(NK_BATCH_SIZE)}, NK_BATCH_SIZE);
        }
        else {
...
void MeshBatcher::customDraw() {
    ...
    if (cullable() && !_is2d) { // 99% only set non cullable on skybox etc.
        ...
        toDraw = _::rawCopy(toDraw, children());
    }
