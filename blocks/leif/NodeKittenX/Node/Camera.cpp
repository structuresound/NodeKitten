...
Camera::Camera(Scene* scene){
    ...
    _scene = scene;
    setPosition(V3(scene->position().x, scene->position().y, scene->position().z - 1));
    name = "camera";
...
void Camera::setTarget(Node *node){
    lookAtNode(node);
}
...
//    [_target runAction:[Action moveToFollowNode:node duration:1.] completion:^{
//     [_target repeatAction:[Action followNode:node duration:1.]];
//     }];
}
...
M16t Camera::viewProjectionMatrix()
{
    if (_vpDirty) {
        _vpDirty = false;
...
M16t Camera::orthographicMatrix() {
    return M16MakeOrtho(-scene()->size().width*.5, scene()->size().width*.5, -scene()->size().height*.5, scene()->size().height*.5, -farZ, farZ);
}
...
void Camera::updateCameraWithTimeSinceLast(F1t dt) {
    Node::updateWithTimeSinceLast(dt);
