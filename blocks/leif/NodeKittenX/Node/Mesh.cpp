...
void Mesh::glInit() {
    ...
    _fboSurface->shader = Shader::shaderNamed(nks(NKS_PASSTHROUGH_TEXTURE_SHADER),{ShaderModule::textureModule(1)});
    
    _fboSurface->_forceOrthographic = true;
    _fboSurface->setUsesDepth(false);
    _fboSurface->_cullFace = GLCullFaceFront;
    _fboSurface->_blendMode = GLBlendModeFBO;
    _fboSurface->_ignoresLights = true;
}
...
void Mesh::fillRect(Scene* context, V4 rect, Color color, shared_ptr<Texture> texture){
    ...
    _fboSurface->setPosition(rect.origin);
    _fboSurface->setSize(V3(rect.size));
    
    _fboSurface->setupShader();
    if (!texture) {
        Shader::fillShader->use();
    }
...
shared_ptr<Mesh> Mesh::nodeWithPrimitive(NKPrimitive primitive, shared_ptr<Texture> texture, Color color, V3t size){
    return make_shared<Mesh>(primitive, texture, color, size);
}

shared_ptr<Mesh> Mesh::nodeWithObj(string objName, V3t size, bool normalize, bool anchor){
    return make_shared<Mesh>(objName, size, normalize, anchor);
}
...
shared_ptr<Mesh> Mesh::nodeWithVbo(shared_ptr<VertexBuffer> buffer, GLenum drawMode, shared_ptr<Texture> texture, Color color, V3t size) {
    return make_shared<Mesh>(buffer, drawMode, texture, color, size);
}
...
Mesh::Mesh(shared_ptr<Texture> texture, Color color, V3t size){
    SizeProperty<_V3t>::setSize(size);
    setAlpha(1.0);
    _color = color;
    setTexture(texture);
}

Mesh::Mesh(NKPrimitive primitive, shared_ptr<Texture> texture, Color color, V3t size):
    Mesh(texture, color, size)
{
        ...
        switch (primitive) {
            ...
            case NKPrimitivePlane:
                _vertexBuffer = VertexBuffer::planeWithSubdivisions((int)(size.x)/4, (int)(size.x)/4);
                _drawMode = GL_TRIANGLE_STRIP;
                setCullFace(GLCullFaceFront);
                break;
...
#warning do video
            ...
            //            }
            //            else {
            shader = Shader::shaderNamed("uColorTextureLightShader",NKS_COLOR_MODE_UNIFORM,_numTextures,1);
            //            }
        }
    }
    else {
        shader = Shader::shaderNamed("uColorLightShader",NKS_COLOR_MODE_UNIFORM,0, 1 );
    }
}
...
void Mesh::setTexture(shared_ptr<Texture> texture){
    ...
    if (texture) {
        ...
        if (!_numTextures && shader){
            chooseShader();
             nkLog("set to some, num existing tex: %lu, choode sh \n", _textures.size());
        }
        _numTextures = 1;
        _textures = {texture};
    }
        ...
        if (_textures.size() && shader) {
            nkLog("set to nil, num existing tex: %lu, choode sh \n", _textures.size());
            chooseShader();
            _textures.clear();
        }
    }
...
#warning DO Video
            ...
            //            }
            //            else {
            if (Shader::activeShader->hasUniformNamed(NKS_TEXTURE_RECT_SCALE)) {
                Shader::activeShader->uniformNamed(NKS_TEXTURE_RECT_SCALE).bindV2(V2MakeF(1.0));
            }
...
void Mesh::customDraw(){
    ...
    if (_color.alpha || _numTextures) {
        
        pushStyle();
        
        if (Shader::activeShader->hasUniformNamed(NKS_V4_COLOR)){
            if (_color.alpha == 0) {
                Shader::activeShader->uniformNamed(NKS_V4_COLOR).bindV4(CLEAR);
            }else {
                Shader::activeShader->uniformNamed(NKS_V4_COLOR).bindV4(glColor());
            }
        }
...
void Mesh::setupViewMatrix() {
    if (!Shader::activeShader) return;
    
    M16t mvp;
    
    if (_forceOrthographic) {
        mvp = M16Multiply(scene()->camera->orthographicMatrix(),
                          M16Multiply(M16MakeLookAt(V3(0, 0, 1), V3(0), V3(0, 1, 0)),
                                      M16ScaleWithV3(globalTransform(), _size)));
    }
    else {
        M16t modelView = M16Multiply(scene()->camera->viewMatrix(),M16ScaleWithV3(globalTransform(), _size));
        
        if(Shader::activeShader->hasUniformNamed(NKS_M16_MV) ){
            Shader::activeShader->uniformNamed(NKS_M16_MV).bindM16(modelView);
        }
        
        if(Shader::activeShader->hasUniformNamed(NKS_M9_NORMAL) ){
            Shader::activeShader->uniformNamed(NKS_M9_NORMAL).bindM9(M16GetInverseNormalMatrix(modelView));
        }
...
void Mesh::drawBoundingBox() {
    ...
    auto tmp = Shader::activeShader;
    
    auto uniformShader = Shader::shaderNamed("boxShader",NKS_COLOR_MODE_UNIFORM,0, 0 );
    
    nkGetGLError();
    
    uniformShader->use();
    Shader::activeShader->uniformNamed(NKS_V4_COLOR).bindV4({1.0,1.0,1.0,1.0});
...
void Mesh::customDrawWithHitShader() {

    Shader::activeShader->uniformNamed(NKS_V4_COLOR).bindV4(Color{uidColor});
