...
FrameBuffer* newEAGLFrameBuffer(EAGLContext *context, id <EAGLDrawable> layer)
...
-(P2t)uiPointToNodePoint:(CGPoint)p {
  // port
  P2t size = P2Make(_sceneController->size.get().width, _sceneController->size.get().height);
  return V2(p.x*_mscale, size.height - (p.y*_mscale));
}

-(void)touchesBegan:(NSSet *)touches withUXEvent:(UIEvent *)theEvent {
...
-(void)touchesMoved:(NSSet *)touches withUXEvent:(UIEvent *)theEvent {
  for (UITouch *t in touches) {
...
-(void)touchesEnded:(NSSet *)touches withUXEvent:(UIEvent *)theEvent {
...
-(void)touchesCancelled:(NSSet *)touches withUXEvent:(UIEvent *)theEvent {
  for (UITouch *t in touches) {
    auto copy = _sceneController->events;
