...
namespace Reactive {
  ...
  template<typename T>
  class Pullable {
    static std::unordered_map<const void *, function<T()>> functions;
  protected:
    mutable bool _dirty;
    mutable T _cache;
  public:
  ...
  template<typename T>
  std::unordered_map<const void *, std::function<T()>> Pullable<T>::functions;

  template<typename T>
  class Variable : public Pullable<T> {
    static std::unordered_map<const void *, std::vector < function<void(void)>>>
    afterFuncs;
  public:
    void afterSet(function<void(void)> &&callback) {
      afterFuncs[this].push_back(callback);
    }

    void set(const T &value) const override {
      Pullable<T>::set(value);
      for (auto &callback : afterFuncs[this]) {
        callback();
      }
    }

    virtual ~Variable() {
      afterFuncs.erase(this);
    }
  };

  template<typename T>
  std::unordered_map<const void *, std::vector < function<void(void)>>>
    Variable<T>::afterFuncs;
