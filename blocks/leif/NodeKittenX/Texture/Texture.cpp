...
shared_ptr<Texture> Texture::blankTexture() {
    return make_shared<Texture>();
}

shared_ptr<Texture> Texture::textureWithImageFile(string filePath, bool relative_path, U1t flags, U1t filter, F1t anistropicFilter) {
    if (nameCache[filePath]) {
...
void Texture::bind() { // GL 1
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(target, glName);
    nkGetGLError();
}
//
void Texture::unbind() { // GL 1
    glBindTexture(target, 0);
    nkGetGLError();
}

void Texture::enableAndBind(int tLocation){
    glActiveTexture(GL_TEXTURE0+tLocation);
    glBindTexture(target, glName);
    nkGetGLError();
}

void Texture::bindToUniform(GLuint uLocation){
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(target, glName);
    glUniform1i(uLocation, 0);
}

void Texture::enableAndBindToUniform(GLuint uSamplerLocation, GLuint tLocation){
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(target, glName);
...
Texture::Texture(string filename, bool relative_path, U1t flags, U1t filter, F1t anistropicFilter, bool cube){
    
    File file {filename, relative_path};
...
{
    File *m = (File*) png_get_io_ptr( structp );
 ...
 */
void Texture::loadPng(File &file)
{
    png_structp structp;
...
void Texture::loadJpg(File &file)
{
...
void Texture::loadPvr(File &file)
{
...
void Texture::allocate(unsigned int flags, unsigned char filter, float anisotropic_filter, bool cube){
...
void Texture::convert16Bits(bool use_5551){
