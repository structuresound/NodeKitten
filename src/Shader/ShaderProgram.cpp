...
namespace Shader {
  ...
  void Program::glInit() {
    fill = Program::shaderNamed("colorFill", NKS_COLOR_MODE_UNIFORM, 0, 0);
    textureFill = Program::shaderNamed("texureFill", NKS_COLOR_MODE_UNIFORM, 1, 0);
  }
  
  void Program::newUIDColorForNode(Node *node) {
  ...
  Node *Program::nodeForColor(UB4t color) {
    return nodeMap[color];
  }
  
  vector<bool> Program::vertexAttributeState{0, 0, 0, 0, 0, 0, 0};
  
  shared_ptr<Program> Program::shaderNamed(string name) {
    auto existing = Program::programCache[name];
    //if (!existing) nkLog("error : no shader prog found for name : %s", name);
    return existing;
  }
  
  shared_ptr<Program> Program::shaderNamed(string name, NKS_COLOR_MODE colorMode, U1t numTextures, int numLights,
                                           int batchSize) {
    
    auto existing = Program::shaderNamed(name);
    if (existing) return existing;
    
    auto newShader = make_shared<Program>(name, colorMode, numTextures, numLights, batchSize);
    
    Program::programCache[name] = newShader;
  ...
  shared_ptr<Program> Program::shaderNamed(string name, vector<ShaderModule> modules, int batchSize) {
    
    auto existing = Program::shaderNamed(name);
    if (existing) return existing;
    
    auto newShader = make_shared<Program>(name, modules, batchSize);
  ...
  Program::Program(string name_, vector<ShaderModule> modules_, int batchSize) : Program() {
  ...
  Program::Program(string name_, NKS_COLOR_MODE colorMode, U1t numTex, int numLights, int batchSize) : Program() {
  ...
  void Program::calculateCommonVertexVaryings() {
...
#else
      _vertMain.push_back(
                          "gl_Position = u_modelViewProjectionMatrix[gl_InstanceID] * vec4(a_position.xy * u_batchScale[gl_InstanceID] + u_batchPosition[gl_InstanceID], a_position.z, 1);");
#endif
    }
  ...
  vector<string> Program::uniformNames() {
...
#warning ATTRIB PROBLEM HERE ? ->
  ...
  bool Program::compileShader(GLuint *shader, GLenum type, string &shaderSource) {
...
#pragma mark - QUERY
  ...
  bool Program::hasAttributeNamed(NKS_ENUM name) {
    for (auto &v : _attributes) {
  ...
  ShaderVariable &Program::attributeNamed(NKS_ENUM name) {
  ...
  bool Program::hasUniformNamed(NKS_ENUM name) {
    for (auto &module : modules) {
  ...
  ShaderVariable &Program::uniformNamed(NKS_ENUM name) {
    for (auto &module : modules) {
      for (auto &v : module.uniforms) {
        if (v.name == name) return v;
      }
  ...
  bool Program::hasVaryingNamed(NKS_ENUM name) {
    for (auto &module : modules) {
  ...
  ShaderVariable &Program::varyingNamed(NKS_ENUM name) {
    for (auto &module : modules) {
      for (auto &v : module.varyings) {
        if (v.name == name) return v;
      }
  ...
  ShaderVariable &Program::vertVarNamed(NKS_ENUM name) {
  ...
  ShaderVariable &Program::fragVarNamed(NKS_ENUM name) {
  ...
  bool Program::use() {
    if (!GLState::setActiveShader(this)) return 0;
