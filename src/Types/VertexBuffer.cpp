...
map<string, shared_ptr<VertexBuffer>> VertexBuffer::vboCache;
map<NKPrimitive, shared_ptr<VertexBuffer>> VertexBuffer::vboPrimitiveCache;

// Factories

shared_ptr<VertexBuffer> VertexBuffer::defaultRect() { // SPRITE
    
    if (VertexBuffer::vboPrimitiveCache[NKPrimitiveRect]) return vboPrimitiveCache[NKPrimitiveRect];
    
    vboPrimitiveCache[NKPrimitiveRect] = make_shared<VertexBuffer>(NKPrimitiveRect);
    
    return vboPrimitiveCache[NKPrimitiveRect];
}

shared_ptr<VertexBuffer> VertexBuffer::axes() { // XYZ
    
    if (VertexBuffer::vboPrimitiveCache[NKPrimitiveAxes]) return vboPrimitiveCache[NKPrimitiveAxes];
    
    vboPrimitiveCache[NKPrimitiveAxes] = make_shared<VertexBuffer>(NKPrimitiveAxes);
    
    return vboPrimitiveCache[NKPrimitiveAxes];
}

shared_ptr<VertexBuffer> VertexBuffer::defaultCube() {
    
    if (VertexBuffer::vboPrimitiveCache[NKPrimitiveCube]) return vboPrimitiveCache[NKPrimitiveCube];
    
    vboPrimitiveCache[NKPrimitiveCube] = make_shared<VertexBuffer>(NKPrimitiveCube);
    
    return  vboPrimitiveCache[NKPrimitiveCube];
}

shared_ptr<VertexBuffer> VertexBuffer::cube8v() {
    
    if (VertexBuffer::vboPrimitiveCache[NKPrimitive8VCube]) return vboPrimitiveCache[NKPrimitive8VCube];
    
    vboPrimitiveCache[NKPrimitive8VCube] = make_shared<VertexBuffer>(NKPrimitive8VCube);
    
    return  vboPrimitiveCache[NKPrimitive8VCube];
}

shared_ptr<VertexBuffer> VertexBuffer::edges8v() {
    
    if (VertexBuffer::vboPrimitiveCache[NKPrimitiveBoundingBox]) return vboPrimitiveCache[NKPrimitiveBoundingBox];
    
    vboPrimitiveCache[NKPrimitiveBoundingBox] = make_shared<VertexBuffer>(NKPrimitiveBoundingBox);
    
    return  vboPrimitiveCache[NKPrimitiveBoundingBox];
}

shared_ptr<VertexBuffer> VertexBuffer::sphereWithStacks(GLint stacks, GLint slices, GLfloat squash)
{
    if (VertexBuffer::vboPrimitiveCache[NKPrimitiveSphere]) return vboPrimitiveCache[NKPrimitiveSphere];
    
    vboPrimitiveCache[NKPrimitiveSphere] = make_shared<VertexBuffer>(stacks, slices, squash);;
    
    return vboPrimitiveCache[NKPrimitiveSphere];
}

shared_ptr<VertexBuffer> VertexBuffer::planeWithSubdivisions(GLint horizontal, GLint vertical){
    
    if (VertexBuffer::vboPrimitiveCache[NKPrimitivePlane]) return vboPrimitiveCache[NKPrimitivePlane];
    
    vboPrimitiveCache[NKPrimitivePlane] = make_shared<VertexBuffer>(horizontal, vertical);
    
    return vboPrimitiveCache[NKPrimitivePlane];
}


VertexBuffer::VertexBuffer(GLint rows, GLint columns){
    ...
    setupVAOInterlaceAndBufferVBO();
    
}


VertexBuffer::VertexBuffer(NKPrimitive primitive, GLenum bufferMode){
...
VertexBuffer::VertexBuffer(GLint stacks, GLint slices, GLfloat squash){
    ...
    // Color
    V4t *vertexColors = (V4t*)calloc(numElements, sizeof(V4t));
...
VertexBuffer::VertexBuffer(F1t width, F1t height, F1t depth, int resX, int resY, int resZ ) {
...
//        memcpy(&elements[i].color, &vertexColors[i], sizeof(V4t));
//    }
//
//    free(vertices);
//    free(vertexNormals);
...
//
//    auto buf = make_shared<VertexBuffer>(sizeof(NKPrimitiveArray)*numElements, numElements, elements, VertexBuffer::primitiveSetupBlock());
//
...
//            memcpy(&elements[i].color, &vertexColors[(i-currentOffset)], sizeof(V4t));
//        }
//
//        free(vertices);
//        free(vertexNormals);
...
//
//
//    VertexBuffer *buf = [[VertexBuffer alloc] initWithSize:sizeof(NKPrimitiveArray)*totalCount numberOfElements:numLevels data:elements setup:[VertexBuffer primitiveSetupBlock]];
//
...
//    VertexBuffer *buf = [[VertexBuffer alloc] initWithSize:sizeof(point) numberOfElements:1 data:point setup:newGeometrySetupBlock{
...
//
//    V4t *colors = (V4t*)calloc(numElements, sizeof(V4t));
//
//    V3t vert;
//    V2t texcoord;
//    V3t normal;
...
//        memcpy(&elements[i].color, &colors[i], sizeof(V4t));
//    }
//
//    free(vertices);
//    free(normals);
...
//
//    VertexBuffer *buf = [[VertexBuffer alloc] initWithSize:sizeof(NKPrimitiveArray)*numElements numberOfElements:numElements data:elements setup:[VertexBuffer primitiveSetupBlock]];
//
//    //buf.drawMode = GL_LINES;
//
//    free(elements);
...
//
//    VertexBuffer *buf = [[VertexBuffer alloc]init];
//
//    buf.lineWidth = 0.1;
//
//    buf.updateBlock = ^(VertexBuffer* buffer){
...
V3t VertexBuffer::normalizeForGroupWithSize(F1t unitSize, V6t groupBoundingBox, bool center) {
...
void VertexBuffer::setupVAOInterlaceAndBufferVBO() {
    ...
    _geometrySetupBlock = [this](){
        
        auto size = _vertices.size();
        
//        nkLog("<VBO>\n *( %d )*", _vertexArray);
//        nkLog("%d elements, %lu bytes, %lu totalBytes %d indices", _stride, _stride*sizeof(F1t), _vertices.size()*sizeof(F1t)*_stride, _indices.size());
//
//        printf("has vertices");
        glEnableVertexAttribArray(NKS_V4_POSITION);
        glVertexAttribPointer(NKS_V4_POSITION, 3, GL_FLOAT, GL_FALSE,
    ...
    interlaceAndBufferVBO();
}

void VertexBuffer::interlaceAndBufferVBO() {
    
    ULt size = _stride * _vertices.size();
    
    nkAssert((size > 0), "attempting to buffer nullptr vertex data \n");
    ...
    for (int i = 0; i < _vertices.size(); i++){
        ...
        if (_colors.size())
            memcpy(interlacedData+(i*_stride)+_colorsOffset, _colors.data()+i, sizeof(V4t));
        if (_tangents.size())
    ...
    bufferVBO(_stride*_vertices.size()*sizeof(F1t), interlacedData);
    
}

void VertexBuffer::bufferVBO(GLsizeiptr size,
                               const GLvoid * data){
...
//        nkLog("new vertex buffer with: %lu vertices, %d bytes", _vertices.size(), size);
//        
//        nkLog("</VBO>");
    }
...
#if NK_SUPPORTS_VAO
void VertexBuffer::bufferEAO(){
    if (_indices.size()){
        if (_elementArray) {
            bindEAO();
            glBufferSubData(GL_ELEMENT_ARRAY_BUFFER,
                            0,
                            _indices.size()*sizeof(U1t),
                            &_indices.front());
        }
        else {
            //nkLog("<EAO>");
            glGenBuffers(1, &_elementArray);
            bindEAO();
            glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                         _indices.size()*sizeof(U1t),
                         &_indices.front(),
                         _bufferMode);
//            nkLog("(%d) id, indices %d, bytes %d, mode %d", _elementArray, _indices.size(), _indices.size()*sizeof(U1t), _bufferMode);
//            nkLog("</EAO>");
...
U1t VertexBuffer::numberOfSegmentsForBezier(B12t* bezier)
{
    F1t segments = B12Length(*bezier);
    return (U1t)ceilf(sqrt(segments * segments * 0.6 + 225.0));
}

SegmentSubdivider VertexBuffer::subdividerForBezier(B12t* bez)
{
    // a p1 p2 b
    V3t a = bez->v[0];
    V3t p1 = bez->v[1];
    ...
    return [=](float t){
        ...
        subdivision.v1 = V3(a.x * nt * nt * nt  +  3.0 * p1.x * nt * nt * t  +  3.0 * p2.x * nt * t * t  +  b.x * t * t * t,
                                a.y * nt * nt * nt  +  3.0 * p1.y * nt * nt * t  +  3.0 * p2.y * nt * t * t  +  b.y * t * t * t,
                                0);
        
        V3t tangent = V3(-3.0 * a.x * nt * nt  +  3.0 * p1.x * (1.0 - 4.0 * t + 3.0 * t * t)  +  3.0 * p2.x * (2.0 * t - 3.0 * t * t)  +  3.0 * b.x * t * t,
                             -3.0 * a.y * nt * nt  +  3.0 * p1.y * (1.0 - 4.0 * t + 3.0 * t * t)  +  3.0 * p2.y * (2.0 * t - 3.0 * t * t)  +  3.0 * b.y * t * t,
                             0);
        
        subdivision.v2 = V3Normalize(V3(-tangent.y, tangent.x, 0));
...
void VertexBuffer::updateBufferWithBezier(V3Stack& bezier , F1Stack& lineWidths){
...
void VertexBuffer::bindEAO() {
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _elementArray);
    nkGetGLError();
};
...
void VertexBuffer::clear(){
    ...
    if (_elementArray){
        glDeleteBuffers(1, &_elementArray);
        _elementArray = 0;
    }
