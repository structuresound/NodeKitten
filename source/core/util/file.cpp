...
void adjust_file_path(string &filePath) {
  unsigned long i = 0, l = filePath.length();
...
string File::appDirectory() { return Platform::appDirectory(); }

string File::pathForResource(string name, string ext) {
  return File::appDirectory() + name + '.' + ext;
}

string File::pathForFileNamed(string name) { return appDirectory() + name; }

/*!
        Open/Extract a file from disk and load it in memory.

        \param[in] filename The file to load in memory.
        \param[in] relative_path Determine if the filename is an absolute or
   relative path.

        \return Return a MEMORY structure pointer if the file is found and
   loaded, instead will return
        NULL.
 */

File::File(string &filename, bool relative_path) {
  ...
  if (unzLocateFile(uf, fname, 1) == UNZ_OK) {
    unzGetCurrentFileInfo(uf, &fi, memory->filename, MAX_PATH, NULL, 0, NULL,
                          0);
    ...
    if (unzOpenCurrentFilePassword(uf, NULL) == UNZ_OK) {
      ...
      memory->buffer =
          (unsigned char *)realloc(memory->buffer, fi.uncompressed_size + 1);
...
/*!
        ...
        \param[in,out] memory A valid MEMORY structure pointer.
        \param[in,out] dst A void pointer to store the bytes that was read from
   this function.
        \param[in] size The number of bytes to read from the memory stream.

        \return Return the number of bytes that the function store in the dst
   pointer.
 */

unsigned long File::read(void *dst, unsigned long size) {
...
/*!
        ...
        \param[in,out] memory A valid MEMORY structure pointer with a loaded
   buffer.
        \param[in] str The string of text to insert.
        \param[in] position The position (in bytes) to inser the text.
 */
void File::insert(char *str, unsigned long position) {
  unsigned long s1 = strlen(str), s2 = _size + s1 + 1;

  char *buffer = (char *)_buffer, *tmp = (char *)calloc(1, s2);
