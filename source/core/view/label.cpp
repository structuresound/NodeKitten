...
Label::Label(string fontName, double fontSize, Color color) :
    ...
    if (!exist) {
        Texture::fontCache[fontName] = make_shared<FontAtlas>(fontName, _loadedSize, 512, 512);
    }
    setTexture(Texture::fontCache[fontName]);

    modelViewMatrix.setPullFunction([this] {
        M16t v = forceOrthographic() ? M16MakeLookAt({0, 0, 1}, {0, 0, 0}, {0, 1, 0})
                                     : scene()->camera->viewMatrix.get();
...
void Label::chooseShader() {
    if (!shader) {
        auto modules = vector<ShaderModule> {ShaderModule::colorModule(NKS_COLOR_MODE_UNIFORM, 0),
                                             ShaderModule::freetypeModule()};
        shader = Program::shaderNamed("freeTypeShader", modules);
//        shader = Program::shaderNamed("freeTypeDebugger",NKS_COLOR_MODE_VERTEX);
    }
}
...
V2t Label::sizeForText() {
    ...
    V2t pen;
    V4t bbox;
    const char *textPtr = _text.c_str();

    texture_font_load_glyphs(_fontAtlas->font, textPtr);

    for (size_t i = 0; i < _text.length(); ++i) {

        texture_glyph_t *glyph = texture_font_get_glyph(_fontAtlas->font, textPtr + i);

        if (glyph != NULL) {
            float kerning = 0.0f;

            if (i > 0) {
                kerning = texture_glyph_get_kerning(glyph, textPtr + (i - 1));
            }
...
void Label::genVertices() {
    ...
    nkAssert(_fontAtlas != 0, "Label::genVertices no font atlas");
    nkAssert(_fontAtlas->font != 0, "Label::genVertices no font for font atlas");
